...
### Note on the use of [Quarto](https://quarto.org/)

Our choice to rely on Quarto has been carefully considered. Leveraging years of experience in literate programming and scientific reporting with Rmarkdown, RStudio (the enterprise behind the RStudio IDE) decided to make python (and julia) communities benefit from it with Quarto as a standalone, language-agnostic publishing tool. We strongly encourage authors to use Quarto, as it offers numerous publishing oriented features, with an extended support for various outputs (online web pages and journal-quality rendered pdf), a simple and centralized workflow, and one of the best-in-class literate programming experience.  
